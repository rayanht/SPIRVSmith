...
from src.operators.arithmetic import (
    OpMatrixTimesMatrix,
    OpMatrixTimesVector,
    OpOuterProduct,
    OpVectorTimesMatrix,
...
class TestArithmetic(unittest.TestCase):
    ...
    def test_matrix_times_matrix_has_correct_shape(self):
        ...
        self.assertEqual(len(matrix_times_matrix.type.type), len(matrix_times_matrix.operand1.type.type))
        # The resulting matrix must have as many columns as the second operand
        self.assertEqual(len(matrix_times_matrix.type.type), len(matrix_times_matrix.operand2.type))